...
### yoseph ###
from utils_yoseph_v2 import load_data # data functions
from utils_yoseph_v2 import sample_box_pose, sample_insertion_pose # robot functions
from utils_yoseph_v2 import compute_dict_mean, set_seed, detach_dict # helper functions
from policy_yoseph_v2 import ACTPolicy, CNNMLPPolicy
...
def main(args):
    ...
    ckpt_dir = args['ckpt_dir'] # '/data2/act/ckpt/fold_v2'
    policy_class = args['policy_class'] # 'ACT'
    onscreen_render = args['onscreen_render'] # False
    task_name = args['task_name'] # 'fold_v2'
...
def make_policy(policy_class, policy_config):
    if policy_class == 'ACT': # True
        policy = ACTPolicy(policy_config) # read policy.py
    elif policy_class == 'CNNMLP':
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    policy = make_policy(policy_class, policy_config)
    policy.cuda()
    optimizer = make_optimizer(policy_class, policy) # just return policy.optimizer

    train_history = []
    validation_history = []
    min_val_loss = np.inf
    best_ckpt_info = None
    for epoch in tqdm(range(num_epochs)): # 2000
        print(f'\nEpoch {epoch}')
        # validation
        with torch.inference_mode():
            policy.eval()
            ...
            for batch_idx, data in enumerate(val_dataloader):
                forward_dict = forward_pass(data, policy) # {'l1': just scalar tensor, 'kl': just scalar tensor, 'loss': just scalar tensor}
                epoch_dicts.append(forward_dict)
            epoch_summary = compute_dict_mean(epoch_dicts) # batch 대상 average... {'l1': just scalar tensor, 'kl': just scalar tensor, 'loss': just scalar tensor}
        ...
        epoch_summary = compute_dict_mean(train_history[(batch_idx+1)*epoch:(batch_idx+1)*(epoch+1)]) # 해당 epoch에서 처리한 batch 대상 average... {'l1': just scalar tensor, 'kl': just scalar tensor, 'loss': just scalar tensor}
