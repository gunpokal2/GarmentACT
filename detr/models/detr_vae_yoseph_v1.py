...
from .transformer_yoseph_v1 import build_transformer, TransformerEncoder, TransformerEncoderLayer
...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names):
        ...
        self.input_proj_env_feat = nn.Linear(256, hidden_dim)
        self.input_proj_garment_feat = nn.Linear(256, hidden_dim)
        self.additional_pos_embed = nn.Embedding(4, hidden_dim)
        ### yoseph ###
    
    ### yoseph ###
    # def forward(self, qpos, image, env_state, actions=None, is_pad=None):
    def forward(self, qpos, image, env_state, env_feature=None, garment_feature=None, actions=None, is_pad=None):
    ### yoseph ###
        """
        qpos: batch, qpos_dim
        ...
        if is_training: # True
            ...
            action_embed = self.encoder_action_proj(actions) # (B, 100, 512) # (bs, seq, hidden_dim)
            ...
            encoder_input = torch.cat([cls_embed, qpos_embed, action_embed], axis=1) # (B, 102, 512) # (bs, seq+1, hidden_dim)
            encoder_input = encoder_input.permute(1, 0, 2) # (102, B, 512) # (seq+1, bs, hidden_dim)
            # do not mask cls token
            cls_joint_is_pad = torch.full((bs, 2), False).to(qpos.device) # (B, 2) all False # False: not a padding
            is_pad = torch.cat([cls_joint_is_pad, is_pad], axis=1) # (B, 102) # (bs, seq+1)
            # obtain position embedding
            pos_embed = self.pos_table.clone().detach() # (1, 102, 512)
            pos_embed = pos_embed.permute(1, 0, 2) # (102, 1, 512) # (seq+1, 1, hidden_dim)
            # query model
            encoder_output = self.encoder(encoder_input, pos=pos_embed, src_key_padding_mask=is_pad) # (102, B, 512)
        ...
        if self.backbones is not None:
            ...
            ### yoseph ###
            env_input = self.input_proj_env_feat(env_feature)
            garment_input = self.input_proj_garment_feat(garment_feature)
            ### yoseph ###
            
            # fold camera dimension into width dimension
            src = torch.cat(all_cam_features, axis=3) # (B, 512, 15, 20)
            pos = torch.cat(all_cam_pos, axis=3) # (1, 512, 15, 20)
            ### yoseph ###
        ...
        a_hat = self.action_head(hs) # (B, 100, 14)
...
def build(args):
    ...
    model = DETRVAE(
        ...
        camera_names=args.camera_names, # ['top']
    )
