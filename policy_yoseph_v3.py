...
from detr.main_yoseph_v3 import build_ACT_model_and_optimizer, build_CNNMLP_model_and_optimizer
import IPython
e = IPython.embed
...
class ACTPolicy(nn.Module):
    ...
    def __call__(
        ...
        if actions is not None: # training time
            ...
            # a_hat (B, 100, 14), mu (B, 32), logvar (B, 32)
            a_hat, is_pad_hat, stage_logits, (mu, logvar) = self.model(
                qpos=qpos,
                image=image,
                env_pcd=env_pcd,
            ...
            a_hat, _, stage_logits, (_, _) = self.model(
                qpos=qpos,
                image=image,
                env_pcd=env_pcd,
                # garment_pcd=garment_pcd,
